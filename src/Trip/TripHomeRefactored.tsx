...
import { RouteTripComment, RouteTripExpenses } from '../Routes/routes';
...
import { TripGettingStarted } from './TripGettingStarted';
import { TripHomeLayout } from './TripHomeLayout';
import { TripPriorityTasks } from './TripPriorityTasks';
import { TripStatusBadge } from './TripStatusBadge';
import { TripTodaySchedule } from './TripTodaySchedule';
...
export function TripHome() {
  ...
  if (!trip || !tripStartDateTime || !tripEndDateTime) {
    return null;
  }

  // Determine if this is a new trip
  const isNewTrip =
    trip.activityIds.length === 0 && trip.taskListIds.length === 0;
  ...
  const heroSection = (
    ...
    >
      <Flex
        ...
        gap="2"
        flexGrow="1"
        maxWidth={{ initial: '100%', sm: isNewTrip ? '100%' : '50%' }}
      >
  ...
  // Primary content varies by trip state
  const primaryContent = (() => {
    if (isNewTrip) {
      return <TripGettingStarted trip={trip} />;
    }

    if (tripStatus?.status === 'current') {
      // During trip: Today's schedule is most important
      return (
        <Flex direction="column" gap="4">
          <TripTodaySchedule trip={trip} todayActivities={todayActivities} />
          <TripPriorityTasks trip={trip} tasks={priorityTasks} maxTasks={3} />
        </Flex>
      );
    }

    // Pre-trip or post-trip: Tasks and upcoming activities
    return (
      <Flex direction="column" gap="4">
        <TripPriorityTasks trip={trip} tasks={priorityTasks} />
  ...
  // Tertiary content - detailed info, less important
  const tertiaryContent = !isNewTrip ? (
